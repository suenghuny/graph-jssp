...
population_size = int(input('Please input the size of population: ') or 1)  # default value is 30
crossover_rate = float(input('Please input the size of Crossover Rate: ') or 0.1)  # default value is 0.8
...
for n in range(num_iteration):
    ...
    for m in range(population_size * 2):
        ...
        print(total_chromosome[m], num_job, num_mc)
...
for m in m_keys:
    for j in j_keys:
        df.append(dict(Task='Machine %s' % (m), Start='2018-07-14 %s' % (str(j_record[(j, m)][0])),
                       Finish='2018-07-14 %s' % (str(j_record[(j, m)][1])), Resource='Job %s' % (j + 1)))
