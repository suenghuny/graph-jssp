...
class Scheduler:
    def __init__(self):
        ...
        self.pt = [list(map(int, self.pt_tmp.iloc[i])) for i in range(self.num_job)] # processing_time
        self.ms = [list(map(int, self.ms_tmp.iloc[i])) for i in range(self.num_job)] # job 별 machine sequence
