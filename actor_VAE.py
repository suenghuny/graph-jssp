...
class PtrNet1(nn.Module):
    def __init__(self, params):
        ...
        self.W_v = nn.ParameterList(
            [nn.Parameter(torch.FloatTensor(2 * params["n_hidden"], 2 * params["n_hidden"])) for _ in
             range(self.n_multi_head)])
        self.W_q = nn.ModuleList(
            [nn.Linear(2 * params["n_hidden"], params["n_hidden"] + params["ex_embedding_size"], bias=False).to(device)
             for _ in range(self.n_multi_head)])
        self.W_k = nn.ModuleList(
            [nn.Linear(2 * params["n_hidden"], params["n_hidden"] + params["ex_embedding_size"], bias=False).to(device)
             for _ in range(self.n_multi_head)])
        self.W_o = nn.ModuleList(
            [nn.Linear(2 * params["n_hidden"], 2 * params["n_hidden"], bias=False).to(device) for _ in
             range(self.n_multi_head)])
    ...
    def init_mask(self):
        ...
        shape1 = torch.tensor(dummy_instance.mask1).to(device).shape[
            1]  # dummy_instance는 shape만 확인해 주기 위해 사용되는 instance
        ...
        for idx in range(len(self.instance)):  # instance의 길이만큼 초기화
            ...
            for i in range(
                    len(instance.mask1)):  # mask1(operation availability)에 대해서, 모든 Job의 첫번째 operation의 availability를 okay로 설정
