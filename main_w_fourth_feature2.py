    ...
    for s in range(epoch + 1, params["step"]):
        ...
        if s % 100 == 1:  # Evaluation 수행
            for p in problem_list:
                ...
                if cfg.vessl == True:
                    ...
                    min_m.to_csv('min_makespan_wo_third_feature.csv')
                    mean_m.to_csv('mean_makespan_wo_third_feature.csv')
        ...
        if s % params["save_step"] == 1:
            torch.save({'epoch': s,
                       ...
                       params["model_dir"] + '/ppo_wo_third_feature' + '/%s_step%d_act.pt' % (date, s))
        #     print('save model...')
...
if __name__ == '__main__':
    ...
    if not os.path.exists(model_dir + "/ppo_wo_third_feature"):
        os.makedirs(model_dir + "/ppo_wo_third_feature")
